...
//     traverse(node, parent){ // 深度遍历
//         if(!node) return;

//         const {type, children}=node;
//         const visit = this.visitor[type];
...
class Transformer{
    constructor(ast){
        ...
        traverse(ast,(current, parent)=>{
            ...
            try {
                switch(type){
                case "text":
                    this.checkTextASTNode(current, parent);
                    break;
                case "element":
                    this.checkElementASTNode(current, parent);
                    break;
                default:
        ...
        if(/\{\{[\s\S]+\}\}/.test(text)){
            ...
            Object.assign(vnode, {
                ...
                text: {
                    raw: text,
                    key: Symbol(),
                    compute: this.functionalize(text)
                },
            });
        }
        context.push(vnode);
            ...
            if(!name){ // illegal attribute name
                ...
                controls[0] = {
                    ...
                    control: {
                        ...
                        computer: this.eval(expression), //new Function("scope", `with(scope){return ${expression}}`)
                    },
                    children: []
                };
    ...
    eval(expression){
        ...
        let code = `function(scope){with(scope){return ${expression}}}`;
        return computers.push(code)-1;
    }

    functionalize(string, type=0){
        const {computers} = this;
        let code = "";
        switch(type){
        case 1: // @for or @if
