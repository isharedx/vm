...
function HTMLParser(html, handler) {
    ...
    function handleStartTag(match) {
        ...
        var attrsLst = match.attrsLst.map(attr => {
            return {
                name: attr[1],
        ...
        // 不是自闭标签
        if (!unary) {
            tagStack.push({ tag: tagName, attrsLst: attrsLst });
        }
        if (handler.start) {
            handler.start(tagName, attrsLst, unary, match.start, match.end);
        }
    }
...
export default function parse(html) {
    ...
    HTMLParser(html, {
        // 处理开始标签
        start(tag, attrsLst, unary) {
            let attrs = attrsLst.reduce((cumulated, { name, value }) => { // [{name: 'class', value: 'xx'}, ...] => [{class: 'xx'}, ...]
        ...
        chars(text) {
            text = (currentParent.tag === 'pre')
                ? text
                : text.trim();//? text : ''
            text && currentParent.children.push({ text })
        }
    })
    return root
}
